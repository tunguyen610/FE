...
namespace Novatic.Controllers
{
    ...
    public class ShopController : BaseController
    {
        ...
        public ShopController(IShopRepository _repository,IAccountRepository _accountRepository , ISystemConfigRepository _repositorySystemConfig, ICacheHelper cacheHelper) : base(cacheHelper)
        {
        ...
        public async Task<IActionResult> List()
        {
            try
            {
                ...
                {
                    dataList = await repository.ListByAccountId(AccountId);
                }
                if (dataList == null || dataList.Count == 0)
                {
                    return NotFound();
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] Shop model)
        {
        ...
        [HttpPost]
        [Route("api/Update")]
        public async Task<IActionResult> Update([FromBody] Shop model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] Shop model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] Shop model)
        {
            int result = 0;
        ...
        {
            int result = repository.CountShop();
            return result;
        }

        [HttpGet]
        [Route("api/List/Top5")]
        public async Task<IActionResult> ListTop5()
        {
            try
            {
                var dataList = await repository.ListTop5();
        ...
        [HttpPost]
        [Route("api/GetListShopName")]
        public async Task<IActionResult> GetListShopName([FromBody] shopIDRequest shopIds)
        {
            if(shopIds == null)
            {
                var novaticResponse = NovaticResponse.NotFoundMesage("please input data");
                //var response = Newtonsoft.Json.JsonConvert.SerializeObject(novaticResponse);
                return Ok(novaticResponse);
            }
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (int x in shopIds.ShopID)
                    {
                        shopNames.Add(repository.GetShopbyID(x).Name);                       
                    }

                    var novaticResponse = NovaticResponse.SUCCESS(shopNames.Cast<object>().ToList());
                    //var response = Newtonsoft.Json.JsonConvert.SerializeObject(novaticResponse);
                    return Ok(novaticResponse);
                }
                catch (Exception ex)
        ...
        [HttpGet]
        [Route("api/GetAccountByShopID")]
        public async Task<IActionResult> GetAccountByShopID(int shopId)
        {
            if (shopId == null)
            {
                var novaticResponse = NovaticResponse.NotFoundMesage("please input data");
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    int accountID = repository.GetShopbyID(shopId).AccountId;
                    Account account = accountRepository.GetAccount(accountID);

                    var novaticReponse = NovaticResponse.SUCCESS(account);
