...
namespace Novatic.Controllers
{
    ...
    {
        IFavouritePostRepository repository;
        ISystemConfigRepository repositorySystemConfig;
        IAccountRepository repositoryAccount;
        public FavouritePostController(ICacheHelper cacheHelper, ISystemConfigRepository _repositorySystemConfig, IFavouritePostRepository _repository, IAccountRepository _repositoryAccount) : base(cacheHelper)
        {
        ...
        [HttpPost]
        [Route("api/LikePost")]
        public async Task<IActionResult> LikePost([FromBody]FavouritePost model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var dataList = await repository.DetailFromUserIDAndPostID(model.AccountId, model.PostId);
                    if (dataList.Count == 0)
                    {
