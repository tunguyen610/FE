...
namespace Novatic.Controllers
{
    ...
    {
        ILanguageConfigRepository repository;
        ISystemConfigRepository repositorySystemConfig;
        IAccountRepository repositoryAccount;
        public LanguageConfigController(ICacheHelper cacheHelper,
            ISystemConfigRepository _repositorySystemConfig, ILanguageConfigRepository _repository, IAccountRepository _repositoryAccount) : base(cacheHelper)
        {
        ...
        public async Task<IActionResult> SetLanguage([FromBody]LanguageConfig model)
        {
            HttpContext.Session.SetString(model.Code, model.Description);
            if (HttpContext.Session.GetString(model.Code) != null)
            {
                var novaticResponse = NovaticResponse.SUCCESS(model);
                return Ok(novaticResponse);
            }
