    ...
    namespace Novatic.Repository
    {
        ...
        {
            NovaticDBContext db;
            public ProductMetaRepository(NovaticDBContext _db)
            {
                db = _db;
            }
            ...
            public async Task<List<ProductMeta>> Detail(int? id)
            {
                if (db != null)
                {
                    try {
            ...
            public async Task<ProductMeta> Add(ProductMeta obj)
            {
                if (db != null)
                {
                    try {
                    object currentUser = System.Security.Principal.WindowsIdentity.GetCurrent().User;

                    await db.ProductMeta.AddAsync(obj);
	...
	db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int? objId)
            {
                ...
                if (db != null)
                {
                    try {
                        ...
                        var obj = await db.ProductMeta.FirstOrDefaultAsync(x => x.Id == objId);
    
                        if (obj != null)
                        {
                            //Delete that obj
                            db.ProductMeta.Remove(obj);
        ...
        public async Task<List<ProductMeta>> ListByProductMeta(int productID)
        {
            if (db != null)
            {
