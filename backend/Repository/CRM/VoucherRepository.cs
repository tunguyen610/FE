...
namespace Novatic.Repository
{
    ...
    {
        NovaticDBContext db;
        public VoucherRepository(NovaticDBContext _db)
        {
            db = _db;
        }
        ...
        public async Task<List<Voucher>> Detail(int? id)
        {
            if (db != null)
            {
                try
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                try
                {
                    ...
                    var obj = await db.Voucher.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null)
                    {
                        //Delete that obj
                        db.Voucher.Remove(obj);
        ...
        public Voucher GetVoucher(string Code)
        {
            if (db != null)
            {
                ...
                try
                {
                    Voucher voucher = (
                        from row in db.Voucher
                        where (row.Active == 1 && row.Code == Code && row.Quantity != 0)                     
                        select row
                    ).First();
                    return voucher;
