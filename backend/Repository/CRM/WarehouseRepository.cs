
using Novatic.Models.CRM;
using Novatic.Models;
using Microsoft.EntityFrameworkCore;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Novatic.Repository
{
    public class WarehouseRepository : IWarehouseRepository
    {
        NovaticDBContext db;
        public WarehouseRepository(NovaticDBContext _db)
        {
            db = _db;
        }


        public async Task<List<Warehouse>> List()
        {
            if (db != null)
            {
                try
                {
                    return await (
                        from row in db.Warehouse
                        where (row.Active == 1)
                        orderby row.Id descending
                        select row
                    ).ToListAsync();
                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return null;
        }


        public async Task<List<Warehouse>> Search(string keyword)
        {
            if (db != null)
            {

                try
                {
                    return await (
                        from row in db.Warehouse
                        where (row.Active == 1 && (row.Name.Contains(keyword) || row.Description.Contains(keyword)))
                        orderby row.Id descending
                        select row
                    ).ToListAsync();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return null;
        }


        public async Task<List<Warehouse>> ListPaging(int pageIndex, int pageSize)
        {
            int offSet = 0;
            offSet = (pageIndex - 1) * pageSize;
            if (db != null)
            {
                try
                {
                    return await (
                        from row in db.Warehouse
                        where (row.Active == 1)
                        orderby row.Id descending
                        select row
                    ).Skip(offSet).Take(pageSize).ToListAsync();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return null;
        }


        public async Task<List<Warehouse>> Detail(int? id)
        {
            if (db != null)
            {
                try
                {
                    return await (
                        from row in db.Warehouse
                        where (row.Active == 1 && row.Id == id)
                        select row)
                    .ToListAsync();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return null;
        }

        public async Task<List<Warehouse>> ListByShopId(int? id)
        {
            if (db != null)
            {
                try
                {
                    return await (
                        from row in db.Warehouse
                        where (row.Active == 1 && row.ShopId == id)
                        select row)
                    .ToListAsync();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return null;
        }

        public async Task<Warehouse> Add(Warehouse obj)
        {
            if (db != null)
            {
                try
                {
                    await db.Warehouse.AddAsync(obj);
                    await db.SaveChangesAsync();

                    return obj;

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return null;
        }


        public async Task Update(Warehouse obj)
        {
            if (db != null)
            {
                try
                {
                    //Update that object
                    db.Warehouse.Attach(obj);
                    // db.Entry(obj).Property(x => x.Name).IsModified = true;
                    // db.Entry(obj).Property(x => x.Description).IsModified = true;
                    // db.Entry(obj).Property(x => x.Active).IsModified = true;
                    db.Entry(obj).Property(x => x.GuId).IsModified = true;
                    db.Entry(obj).Property(x => x.ShopId).IsModified = true;
                    db.Entry(obj).Property(x => x.WarehouseTypeId).IsModified = true;
                    db.Entry(obj).Property(x => x.WarehouseStatusId).IsModified = true;
                    db.Entry(obj).Property(x => x.Active).IsModified = true;
                    db.Entry(obj).Property(x => x.Name).IsModified = true;
                    db.Entry(obj).Property(x => x.Description).IsModified = true;
                    db.Entry(obj).Property(x => x.ContactPerson).IsModified = true;
                    db.Entry(obj).Property(x => x.Location).IsModified = true;
                    db.Entry(obj).Property(x => x.Email).IsModified = true;
                    db.Entry(obj).Property(x => x.Phone).IsModified = true;
                    db.Entry(obj).Property(x => x.AddressCity).IsModified = true;
                    db.Entry(obj).Property(x => x.AddressDistrict).IsModified = true;
                    db.Entry(obj).Property(x => x.AddressWard).IsModified = true;
                    db.Entry(obj).Property(x => x.AddressDetail).IsModified = true;


                    //Commit the transaction
                    await db.SaveChangesAsync();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }
        }


        public async Task Delete(Warehouse obj)
        {
            if (db != null)
            {
                try
                {
                    //Update that obj
                    db.Warehouse.Attach(obj);
                    db.Entry(obj).Property(x => x.Active).IsModified = true;

                    //Commit the transaction
                    await db.SaveChangesAsync();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }
        }

        public async Task<int> DeletePermanently(int? objId)
        {
            int result = 0;

            if (db != null)
            {
                try
                {
                    //Find the obj for specific obj id
                    var obj = await db.Warehouse.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null)
                    {
                        //Delete that obj
                        db.Warehouse.Remove(obj);

                        //Commit the transaction
                        result = await db.SaveChangesAsync();
                    }
                    return result;

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return result;
        }


        public int CountWarehouse()
        {
            int result = 0;

            if (db != null)
            {
                try
                {
                    //Find the obj for specific obj id
                    result = (
                        from row in db.Warehouse
                        where row.Active == 1
                        select row
                    ).Count();

                }
                catch (Exception e)
                {
                    string error = e.Message;
                }
            }

            return result;
        }
    }
}

